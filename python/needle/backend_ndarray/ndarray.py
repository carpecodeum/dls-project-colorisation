...
class NDArray:
    ...
    def reshape(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        if prod_old != prod_new:
            raise ValueError(f"Cannot reshape array of size {prod_old} into shape {new_shape}")
    ...
    def broadcast_to(self, new_shape: tuple[int, ...]) -> "NDArray":
        ...
        for i in range(len(new_shape)):
            if i < ndim_diff:
                continue
            idx = i - ndim_diff
            if self.shape[idx] == 1 and new_shape[i] != 1:
                new_strides[i] = 0
            elif self.shape[idx] != new_shape[i]:
    ...
    def __getitem__(self, idxs: int | slice | tuple[int | slice, ...]) -> "NDArray":
        ...
        for i, s in enumerate(slices):
            ...
            size = (s.stop - s.start + s.step - 1) // s.step
            if size > 1 or (size == 1 and len(slices) > 1):
                new_shape.append(size)
                new_strides.append(self.strides[i] * s.step)
            elif size == 1 and len(slices) == 1:
                new_shape.append(size)
                new_strides.append(self.strides[i] * s.step)
