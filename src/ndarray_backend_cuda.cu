...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    for (int i = shape.size - 1; i >= 0; i--) {
      size_t dim_idx = idx % shape.data[i];
      idx /= shape.data[i];
      in_idx += dim_idx * strides.data[i];
    }
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
    ...
    for (int i = shape.size - 1; i >= 0; i--) {
      size_t dim_idx = idx % shape.data[i];
      idx /= shape.data[i];
      out_idx += dim_idx * strides.data[i];
    }
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out,
  ...
  // Use 2D grid: each thread computes one element of the output matrix
